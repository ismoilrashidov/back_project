const { Venue_venuetype, Venue } = require("../models");
const { validateVenue_venuetype } = require("../validations/venue_venuetypeValidation");
...
exports.updateVenue_venuetype = async (req, res) => {
    ...
    if (error) return res.status(400).send(error.details[0].message);
    try {
        const venue_venuetype = await Venue_photo.findByPk(req.params.id);
        if (!venue_venuetype) return res.status(404).send("Venue_venuetype not found");
