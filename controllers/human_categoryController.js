...
const { validateHuman_category } = require("../validations/human_categoryValidation");
...
exports.getHuman_categories = async (req, res) => {
    try {
        const human_categories = await Human_category.findAll();
        res.status(200).send(human_categories);
    } catch (error) {
